...
impl SequencerProver<VestaConfig, PallasConfig, PallasConfig> for InMemProver {
    fn rollup_proof(
        client_inputs: [plonk_prover::rollup::circuits::client_input::ClientInput<VestaConfig, 1, 1, 8>;
            2],
        global_vk_root: Fr,
    ...
    > {
        let (mut circuit, _pi_star) = base_rollup_circuit::<VestaConfig, PallasConfig, 2, 1, 1, 8>(
            client_inputs,
            global_vk_root,
